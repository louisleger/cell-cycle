...
def plot_loss(configs = ()):
    ...
    ax[0].set_title('Train Loss'); ax[1].set_title('Test Loss'); ax[0].set_ylabel('$\ell_2$', fontsize = 14, rotation=0, labelpad=10); ax[0].legend()
    ax[0].set_xlabel('Epochs');  ax[1].set_xlabel('Epochs'); ax[0].set_ylim(0, max(data['train_loss'])); ax[1].set_ylim(0, max(data['train_loss']))
...
def plot_normalized_time_error(error): 
    mean = error.mean(axis= 0); std = error.std(axis=0)
    tau = np.linspace(0, 1, error.shape[1])
    fig, axes = plt.subplots(1, 2, figsize=(10, 4), sharey=True)

    for i, signal in enumerate([1, 0]): 
        axes[i].plot(tau, mean[:, signal], label='$\mu$', color=['green', 'red'][signal])
        axes[i].fill_between(tau, mean[:, signal] - std[:, signal], mean[:, signal] + std[:, signal], alpha=0.2, label='$\sigma$', color=['green', 'red'][signal])
        
        axes[i].set_xlabel('$\\tau$', fontsize=14); axes[i].set_ylabel('$\ell_1$', rotation=0, fontsize=14, labelpad=10);
        axes[i].set_title(f"$y_{['g', 'r'][signal]}$", fontsize=16); axes[i].legend()
