...
def get_cells_collection(mask_filenames, well_id, cells_of_interest):
    ...
    for file in mask_filenames:
        if file.split(".")[1] == "tif":
            ...
            background = np.mean(I[frame_idx], axis=(1, 2))
            background_noise = np.std(I[frame_idx], axis=(1, 2))
            for cell_id in frame_cells:
                if cell_id not in cell_stats_collection.keys():
                    cell_stats_collection[cell_id] = {
                        "AREA": [],
                        "POSITION": [],
                        "RADIUS": [],
                        "CHANNEL_SUM": [],
                ...
                distances = np.sqrt(
                    (rows - position[0]) ** 2 + (cols - position[1]) ** 2
                )
                ...
                nucleus_values = np.round(
                    (I[frame_idx, :, rows, cols] - background[np.newaxis, :])
                    / background_noise[np.newaxis, :],
                    2,
                ).astype(np.float16)
                cell_stats_collection[cell_id]["CHANNEL_SUM"].append(
                    nucleus_values.sum(axis=0)
                )
                cell_stats_collection[cell_id]["CHANNEL_MEAN"].append(
                    np.around(nucleus_values.mean(axis=0), 2)
                )
                ...
                raw_img = I[
                    frame_idx,
                    IMG_CHANNELS,
                    int(position[0] - CELL_SIZE / 2) : int(position[0] + CELL_SIZE / 2),
...
def optimized_expand_ctc_results(well_id, save_path):
    ...
    ctc_results_directory = TRACKMATE_PATH + f"{well_id}/01_RES/"
    res_track = pd.read_csv(
        ctc_results_directory + "res_track.txt",
        sep=" ",
        header=None,
        names=["CELL_ID", "T_START", "T_END", "PARENT"],
    )
    print(
        "Results from Tracking, Edge List Dimensions:", res_track.shape
    )  # (number of cells/tracks, 4)

    # Filter out tracks that are very short with MIN_TRACK_LENGTH constant above
    res_track["DELTA_T"] = res_track.apply(
        lambda row: row["T_END"] - row["T_START"], axis=1
    )
    ...
    print(
        f"Tracks remaining after time filtering = {round(len(filtered_tracks)/len(res_track), 2)} %"
    )
    ...
    for root, dirs, files in sorted(os.walk(ctc_results_directory)):
        mask_filenames = sorted(files)
    n_cells_per_frame = (
        pd.DataFrame({1: np.zeros(len(mask_filenames), dtype=int)})
        .merge(
            pd.DataFrame(
                filtered_tracks.apply(
                    lambda cell: np.arange(cell["T_START"], cell["T_END"] + 1), axis=1
                )
                .explode()
                .reset_index()
                .groupby(0)
        ...
        .apply(lambda ncells: ncells[0] + ncells[1], axis=1)
        .values.tolist()
    )
    ...
    lst_cell_collections = pool.map(
        process_partition,
        [(partition, well_id, cells_of_interest) for partition in partitions],
    )
    ...
    for key in stat_names:
        filtered_tracks[key] = filtered_tracks["CELL_ID"].apply(
            lambda cid: (
                cell_stats_collection[cid][key]
                if cid in cell_stats_collection.keys()
                else np.nan
            )
        )
    ...
    for c in cells:
        exploded_df = (
            filtered_tracks[filtered_tracks["CELL_ID"] == c]
            .explode(
                column=[
                    "FRAME",
                    "AREA",
    ...
    channel_df = result_df[
        ["CELL_ID", "FRAME"] + column_types["channel"] + column_types["information"]
    ]
...
if __name__ == "__main__":
    ...
    wells = sorted(
        [
            well.split(".")[0]
            for well in os.listdir(PATH[EXPERIMENT])
            if well.endswith(".tif")
        ]
    )
    for well_id in wells:
        ...
        end_time = time.time()
        print(
            f"Done well {well_id} in {(end_time - start_time)/60} minutes, file: {SAVE_PATH}"
        )
    print(f"All Done!")
