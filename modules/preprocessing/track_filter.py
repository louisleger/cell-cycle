...
wells_dictionnary = {
    ...
    "drugged": [f"{i:02d}" + f"{j:02d}" for i in range(3, 6) for j in range(4, 12)]
    + [f"{i:02d}" + f"{2:02d}" for i in range(3, 7)]
    + ["0706", "0707", "0708", "0709"],
}
...
full_tracks = set(
    pd.read_csv(
        "/home/maxine/Documents/louis/perfect_repo/full_tracks.csv", dtype="str"
    )["CELL_ID"].tolist()
)
...
def filter_exploded_dataframes(well, save_path):
    ...
    if not os.path.exists(EXTRACTION_DIRECTORY + well + "/channels.pkl"):
        return well
    ...
    channels["fucci1"] = channels.apply(
        lambda cell: cell["CHANNEL_SUM"][1] / cell["AREA"], axis=1
    )
    channels["fucci2"] = channels.apply(
        lambda cell: cell["CHANNEL_SUM"][2] / cell["AREA"], axis=1
    )
    epsilon = np.array(
        [
            np.percentile(channels["fucci1"].values, q=1),
            np.percentile(channels["fucci2"].values, q=1),
        ]
    )
    softplus = lambda x, beta=1: np.where(
        beta * x < 20, (1 / beta) * np.log(1 + np.exp(beta * x)), x
    )
    channels["fucci1"] = channels["fucci1"].apply(
        lambda f1: np.log2(softplus((f1 - epsilon[0]) / epsilon[0]))
    )
    channels["fucci2"] = channels["fucci2"].apply(
        lambda f2: np.log2(softplus((f2 - epsilon[1]) / epsilon[1]))
    )

    df = pd.merge(
        channels[["CELL_ID", "FRAME", "fucci1", "fucci2", "POSITION", "PARENT"]],
        images[["CELL_IMG"]],
        left_index=True,
        right_index=True,
    )
    ...
    df["correct_size"] = df["CELL_IMG"].apply(
        lambda im: (
            True if im.shape[1] == cell_size and im.shape[2] == cell_size else False
        )
    )
    border_cells = np.unique(df[df["correct_size"] == False]["CELL_ID"].values)
    df = df[~df["CELL_ID"].isin(border_cells)].reset_index(drop=True)
    print(
        "Cells w/ wrong image sizes (border cells):",
        len(border_cells),
        "or",
        round(len(border_cells) / number_of_cells, 2),
        "%",
    )
    ...
    cell_life = pd.DataFrame(
        df.groupby("CELL_ID").apply(
            lambda group: pd.Series(
                {
                    "frames": group["FRAME"].values.tolist(),
                    ...
                    "PARENT": group["PARENT"].values.tolist()[0],
                }
            )
        )
    ).reset_index()
    ...
    if minmax:
        print("Min-Max")
        cell_life["fucci1"] = cell_life["fucci1"].apply(
            lambda f1: mm.fit_transform(np.array(f1).reshape(-1, 1))
        )
        cell_life["fucci2"] = cell_life["fucci2"].apply(
            lambda f2: mm.fit_transform(np.array(f2).reshape(-1, 1))
        )

    # Getting tau the normalized time, tau = time/track_length = t/T
    cell_life["tau"] = cell_life["frames"].apply(
        lambda frame_list: np.linspace(0, 1, len(frame_list))
    )
    print(
        "Total Trainable Cells:",
        len(cell_life),
        "or",
        round(len(cell_life) / number_of_cells, 2),
        "%",
    )
    ...
    metadata = {
        ...
        "POSITION": [
            np.array(p).tolist() for p in cell_life["POSITION"].values.tolist()
        ],
    }
    ...
    cell_life.apply(
        lambda track: np.save(
            f"{save_path}images/{track['CELL_ID']}.npy", arr=np.array(track["CELL_IMG"])
        ),
        axis=1,
    )
    ...
    cell_life.apply(
        lambda track: np.save(
            f"{save_path}labels/{track['CELL_ID']}.npy",
            arr=np.stack([track["fucci1"], track["fucci2"]]),
        ),
        axis=1,
    )
    return well
